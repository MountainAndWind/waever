...
checkCustomize = function (){
    ...
    /*		var rows = jQuery("#nodesnum0").val();  //明细表行数
            var idvalues = jQuery("#submitdtlid0").val(); //明细表具体行id
            var k=0;
            var data=new Array();
            ...
            for(var i=0;i<rows;i++ ){
                var hotelval = jQuery("#" + hotel + data[i]).val()*1.0; //实际每行住宿费
                var dateval = jQuery("#" + standarthotel + data[i]).val()*1.0;  //实际标准住宿费
                    if(hotelval > dateval){
                        var hang =parseInt(data[i])+1;
    ...
    */
    ...
    if (eles) {
        for (var i = 0; i < eles.length; i++) {
            ...
            var hotelval= jQuery("#"+hotel+currentIndex).val()*1.0;
            var dateval = jQuery("#"+standarthotel+currentIndex).val()*1.0;
...
jQuery("#field6311").bindPropertyChange(function (e) {
    getFwh();
});
function GetYus(){
