...
//合计强制计算
    ...
    checkCustomize = function(){
        ...
        var hzJe_field="#field9516";//发票汇总金额
...
function resetHjData() {
    ...
    for (var i = 0; i < num.length; i++) {
        ...
        if(isExist(trainIds,invoiceTypeVal)){//火车字段
            trainCount=Number(trainCount)+Number(jshjVal);
        }
        if(isExist(transIds,invoiceTypeVal)){//交通费 transIds
            transCount=Number(transCount)+Number(jshjVal);
        }
        if(isExist(luQiaoIds,invoiceTypeVal)){//路桥费
            luQiaoCount=Number(luQiaoCount)+Number(jshjVal);
        }
        if(isExist(accommodationIds,invoiceTypeVal)||("2"==invoiceTypeVal&&zyVal.indexOf("住宿")&&!isNull(zyVal))){//住宿费
            accommodationCount=Number(accommodationCount)+Number(jshjVal);
        }
        if(isExist(canYingIds,invoiceTypeVal)&&zyVal.indexOf("餐饮")&&!isNull(zyVal)){//餐饮
...
function showInfo() {
    ...
    var fields="#field9099,#field9484,#field9523,#field9486,#field9487,#field9108";
    var mainFields="#field6557,#field6009,#field6010";
    var errField="#field9527";
    var index=6;
