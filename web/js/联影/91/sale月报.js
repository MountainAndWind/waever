    ...
    checkCustomize = function (){
        ...
        for (var i = 0; i < num.length; i++) {
            ...
            if(""!=type){
                ...
                if($("#"+bxlx).val()!="0"){//非个人报销  不控市内交通和招待费
                        if(isExist(groupIds,type)){
                    ...
                    if(isExist(serveIDs,type)||isExist(indoorTransEdIDs,type)||isExist(indoorTransIDs,type)||isExist(groupIds,type)){
                        //}else if(isExist(serveIDs,type)||isExist(groupIds,type)){
                        if(Number(sbJe)>Number(limitJe)){
                            top.Dialog.alert("明细第"+index+"行无法提交，实报金额不得超过执行标准！！！")
                            return false;
                        }
