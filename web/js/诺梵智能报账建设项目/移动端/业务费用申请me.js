    ...
    if (val != "") {
        for (var i = 0; i < num.length; i++) {
            if(index!=num[i]){
                ...
                if (val == vali) {
                    alert("发票选择与第" + getIndex(0,num[i]) + "行重复，请重新选择发票！");
                    $(fphmField+"_"+index).val("")
                    setDetailVal(fphmField,0,index,"")
...
$(document).ready(function(){
    ...
    /* var num = getDetialIndexArr(0);
     ...
     jQuery("#nodenum0").bindPropertyChange(function(obj){
         var index = parseInt(jQuery(obj).val())-1;//获取新增明细下标
         if(window.console) console.log("index--->"+index);
         //绑定明细列字段，明细会先解析出field6307_0  当点击明细行时，会在解析出field6307_0_d
         //给明细列字段绑定blur change 等事件无效
         $(fphmField + "_" + index).bindPropertyChange(function (e) {
             var rowNumIndex = e.id.split("_")[1]
             validationFphm(rowNumIndex)
         });
     })*/
